...
the existing panel outputs, for space-efficient Results tables.
...
def main():
    ...
    parser.add_argument("--no_cross_fig", action="store_true",
    ...
    baseline_df = panel.loc[panel["year"] == args.year, baseline_cols]
    ...
    # -------------------------
    # 2) Extraction Figures (keep these)
    # -------------------------
    ...
    if "extraction_total_Tcc" in panel.columns:
        fig = plot_timeseries(
            panel, "year", "extraction_total_Tcc",
            "Total extraction over time (Tcc)",
            "Extraction total (Tcc, $bn)"
        )
    ...
    if "extraction_total_Tii" in panel.columns:
        fig = plot_timeseries(
            panel, "year", "extraction_total_Tii",
            "Total extraction over time (Tii)",
            "Extraction total (Tii, $bn)"
        )
    ...
    link_extr_labels = {
        ...
        "extraction_total_Tcc": "Extraction (commodity system, $bn)",
        "extraction_total_Tii": "Extraction (industry system, $bn)",
    }
    ...
    if made_extraction_tcc:
        snippet_lines += [
            ...
            r"\caption{Total hypothetical extraction over time under the commodity-by-commodity representation (Tcc).}",
    ...
    if made_extraction_tii:
        snippet_lines += [
            ...
            r"\caption{Total hypothetical extraction over time under the industry-by-industry representation (Tii).}",
